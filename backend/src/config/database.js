...
async function createDBPool(retries = 5, delay = 2000) {
    ...
    for (let i = 0; i < retries; i++) {
        try{
            ...
            if (i < retries - 1) {
                log.warning(`Database connection pool creation failed (Attempt ${i + 1}/${retries}). Try again in ${delay} ms. Error: ${error.message}`);
                await new Promise(res => setTimeout(res, delay));
            } else {
                log.error(`Database connection pool creation failed after ${retries} attempts. Error: ${error.message}`);
                throw error;
            }
        }   
    }
