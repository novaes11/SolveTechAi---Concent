...
const createPerson = async (req, res) => {
    ...
    const {name, biological_sex, date_of_birth} = req.body;

    // data validation - check if all required fields are present
    if(!name || !biological_sex || !date_of_birth){
        log.warning('Missing required fields in request body');
        return res.status(400).json({error: 'Missing required fields: name, biological_sex, date_of_birth'});
    }

    // data validation - check if biological_sex if valid (M or F)
    if (biologica_sex.length !== 1 || !['M', 'F'].includes(biological_sex.toUpperCase())) {
    ...
    try {
        log.debug(`Creating person with name: ${name}, biological_sex: ${biological_sex}, date_of_birth: ${date_of_birth}...`);
        const newPerson = await PersonModel.createPerson(name, biological_sex, date_of_birth);
        log.info(`Person created successfully with ID: ${newPerson.id}`);
